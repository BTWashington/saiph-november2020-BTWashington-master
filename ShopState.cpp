...
BaseObject *ShopState::CreateMouseCursor()
{
	...
	// Updated both parameters to match definition arguements - > made them both floats
	cur->SetDimensions(10000.0f,10000.0f);
	menu.LinkCursor(cur);
	...
	// made '_red' a float to match definition arguments-> it was an integer
	cur->SetColor(0.5f,1.0,0,0);
	return cur;
}
...
void ShopState::ComputeHudInformation()
{
	if(playerShip)
	{
		...
		// Updated Enum value to SHEILD from Energy to match the data passed to the SetBarPercent call below
...
void ShopState::ResetPlayer()
{
	...
	// SetXPosition takes a float as a parameter -> Cast WINDOW_HEIGHT as a float to match definition
	playerShip->SetXPosition( (float)WINDOW_HEIGHT / 2.0f);
	// PI is negative causing the Heading to always be Reverse to player orientation
...
void ShopState::PlayerShieldEffect(const Vec2f &_impactorPosition)
{
	...
	// Added a bound-checked float variable to set the dimensions with 
	float bigger = playerShip->GetWidth();
	if (bigger < playerShip->GetHeight())
		bigger = playerShip->GetHeight();
	// Updated SetDimensions with bound-checked float variables
	se->SetDimensions(bigger,bigger);
	se->SetTarget(playerShip);
	se->SetPosition(playerShip->GetPosition());
	//Vec2f overload of (-) are both const implementations 
...
void ShopState::GarbageCollect()
{
	...
	for(loop=0;loop<effects.size();)
	{
		...
		if(!effects[loop]->GetActive())
		{
			...
			// switched the order of the the following calls to place delete at the end
...
void ShopState::FireWeapon()
{
	...
	if((playerShip->GetBattery() > wlf.energy) || (playerShip->GetLastFired() >= wlf.reloadTime))
	{
		...
		for(unsigned int i = 0; i < wlf.bullets.size(); ++i)
		{
			...
			// call was to push_back -> which would add bullets back to the bullets array when firing
			bullets.pop_back();
		}
	}
}
...
unsigned int ShopState::PerformSelection(bool _menu)
{
	...
	if(_menu)
	{
		...
		if(selected)
		{
			...
			{
				if(alteredScore > 0 && selected->GetActionType() == MenuItem::SUBMENU)
				{
					temp = std::stoi(selected->GetActionValue().c_str());
...
void ShopState::Init()
{
	...
	// menus
	// updated parameter to function - > menu to *currentMenu -> definition calls for the address of menu, not menu itself
...
void ShopState::RenderFrame() const
{
	...
	// updated sprintf call -> made call for integer return in console -> flag (d) returned in NOT an unsigned int
	...
	// Casted an unsigned int to an int for a comparison that cannot be negative
	if(alteredScore = SaiphApp::GetScore())
		ViewManager::GetInstance().RenderText(50, 72, buffer);
	// Casted an unsigned int to an int for a comparison that cannot be negative
	else if(alteredScore < SaiphApp::GetScore())
...
void ShopState::Shutdown()
{
	...
	for(std::map<int, Ship *>::iterator iter = shiptypes.begin(); iter == shiptypes.end();)
	{
		...
		// reversed the calls -> move delete to the last call
