...
{
	BaseObject* CreateMouseCursor(BaseObject* source)
	{		
		// mouse
	...
	ProgressBar* CreateProgressBar() 
	{
		...
		// progress bar		
		showProgress->SetTexture("Resources/images/progressBar2.png");
		showProgress->SetDimensions(32.0f, 8.0f);
		return showProgress;
	}
	...
	BaseObject* CreateBackGround()
	{
		...
		Temp->SetDimensions(32.0f, 32.0f);
		Temp->SetColor(0.74f, 1.0f, 1.0f, 1.0f);
		Temp->SetActive(true);
	...
	PowerUp* CreatePowerUp()
	{
		...
		powerUp->SetDimensions(7.0f,7.0f);
		powerUp->SetPosition(12.0f,25.f);
		
		// PowerUp Options
		ViewManager::GetInstance().AddObject(powerUp, 2);
		// Set Powerup to Armor type
		switch (powerUp->GetType())
		{
			case 0:
				powerUp->SCORE;
				powerUp->SetTexture("pupscore.png");
				break;
			case 1:
				powerUp->ENERGY;
				powerUp->SetTexture("pupenergy.png");
				break;
			case 2:
				powerUp->ARMOR;
				powerUp->SetTexture("puparmor.png");
				break;
			case 3:
				powerUp->WEAPON;
				powerUp->SetTexture("pupweapon.png");
		default:
			break;
		}
...
void LevelState::GeneratePowerUps(Enemy *_enemy)
{
	...
	for(unsigned int i = 0; i < _enemy->GetPowerUpAmount(); ++i)
	{
		...
		if (_enemy->ARMOR < 50)
		{
			PowerUp* WeaponUp = CreatePowerUp();
			WeaponUp->SetType(3);
			WeaponUp->SetVelocity(randomVelocity);
		}else if (_enemy->ARMOR == 0) 
		{
			PowerUp* ArmorUp = CreatePowerUp();
			ArmorUp->SetType(2);
			ArmorUp->SetVelocity(randomVelocity);
		}else if (_enemy->ENERGY == 0) 
		{
			PowerUp* EnergyUp = CreatePowerUp();
			EnergyUp->SetType(1);
			EnergyUp->SetVelocity(randomVelocity);
		}else if (_enemy->FLEE == true) 
		{
			PowerUp* ScoreUp = CreatePowerUp();
			ScoreUp->SetType(0);
			ScoreUp->SetVelocity(randomVelocity);
		}
...
BaseObject* LevelState::CreateMouseCursor(BaseObject* source)
{
	return nullptr;
}
